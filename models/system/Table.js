...
Table.prototype.loadStructure = function (cb) {
    ...
    async.waterfall([
            ...
            function (tables, cb) {
                ...
                _t.structure.created = {type:"datetime", name:'Создан'+_t.profile.ending};
                _t.structure.updated = {type:"datetime", name:'Обновлен'+_t.profile.ending};
                _t.structure.deleted = {type:"datetime", name:'Удален'+_t.profile.ending};
                _t.structure.published = {type:"datetime", name:'Опубликован'+_t.profile.ending};
    ...
    async.series([
        function (cb) {
            ...
            _t.checkExist(function (err, info) {
                if (err) return cb(new MyError('Не удалось проверить существование таблицы в базе. ',{table:_t.name, err:err}));
                if (!info){
                    ...
                    async.waterfall([
                        ...
                        function (conn, cb) {
                            conn.fields(_t.name, function(err, fields) {
                                conn.release();
                                if (err) return cb(new MyError('Не удалось загрузить инфу о столбцах таблицы.', {table:_t.name, err:err}));
                                        ...
                                        async.waterfall([
                                            ...
                                            function (conn, cb) {
                                                ...
                                                conn.query(sql, [_t.name], function (err, res) {
                                                    conn.release();
                                                    if (err) return cb(new MyError('Не удалось добавить поле',{table:_t.name,column:key,err:err}));
                                                    cb(null);
                                                });
                                            }
                                        ],cb)
                                        ...
                                        if (columnType !==fieldFromFile.type || columnLength!==fieldFromFile.length){
                                            ...
                                            async.waterfall([
                                                ...
                                                function (conn, cb) {
                                                    ...
                                                    conn.query(sql, [], function (err, res) {
                                                        conn.release();
                                                        if (err) return cb(new MyError('Не удалось обновить тип поля',{table:_t.name,column:key,type:type,err:err}));
                                                        cb(null);
                                                    });
                                                }
                                            ],cb)
                                    ...
                                    async.waterfall([
                                        ...
                                        function (conn, cb) {
                                            conn.update("class_profile",toModyfy, function (err, res) {
                                                conn.release();
                                                if(err) return cb(new MyError('Не удалось обновить поля class_profile',{err:err,toModify:toModyfy}));
                            ...
                            function (cb) {
                                ...
                                async.waterfall([
                                    ...
                                    function (conn, cb) {
                                        ...
                                        var sql = "select MAX(sort_no) from class_fields_profile where class_id = ?";
