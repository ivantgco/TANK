...
socket.on('socketQueryCallback', function (callback_id, result, request_time) {
    ...
    if (typeof result==='object' && result!==null){
        ...
        var t = result.toastr;
        if (typeof toastr=="object" && t) toastr[t.type](t.message, t.title);
    ...
    if (typeof item.callback === "function") {
        ...
        if(result.code == 10){
            ...
            toastr[result.toastr.type](result.toastr.message + '<button id="confirm_socket_query_'+btnGuid+'" type="button" class="btn clear">Подтвердить</button> <button id="cancel_socket_query_'+btnGuid+'" type="button" class="btn clear">Отмена</button>','',{
                "closeButton": false,
                "debug": false,
                "newestOnTop": false,
                "progressBar": false,
            ...
            $('#cancel_socket_query_'+btnGuid).off('click').on('click', function(){
                toastr['error']('Операция отменена');
                window.setTimeout(function(){
                    toastr.clear();
                }, 1000);
